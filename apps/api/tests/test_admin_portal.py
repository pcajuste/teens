...
def _seed_rep(db, *, age: int = 20) -> tuple[str, str]:
    ...
    db.execute(
        """
        INSERT INTO public.talent_profiles (id, user_id, display_name, school_name, city, state, graduation_year, categories)
        VALUES ($1, $2, 'Test Talent', 'Test High', 'Austin', 'TX', 2027, '{gaming}')
        """,
        talent_id,
        talent_user_id,
    )
    return talent_user_id, talent_id
    ...
    *,
    ...
    db.execute(
        ...
        INSERT INTO public.campaign_talents
            (id, campaign_id, talent_id, status, payout_status, payout_cents,
...
def test_queue_reps_is_always_empty(client, db, admin_headers):
    # Talents never sit in an admin-approval pending state (see
    ...
    # approve/reject 400s for talent account_type.
    response  = client.get("/admin/queue/talents", headers=admin_headers)
    assert response .status_code == 200
...
def test_talent_type_is_not_admin_reviewable(client, db, admin_headers):
    _, talent_id = _seed_rep(db)
    response  = client.post(f"/admin/approve/talent/{talent_id}", headers=admin_headers)
    assert response .status_code == 400
    assert response .json()["error"]["code"] == "talents_not_admin_reviewed"
...
def test_stuck_payments_includes_49h_row_excludes_40h_row(client, db, admin_headers):
    _, brand_id = _seed_active_brand(db)
    campaign_id = _seed_campaign(db, brand_id)
    _, stuck_talent_id = _seed_rep(db)
    _, fresh_talent_id = _seed_rep(db)
    ...
    assert response .status_code == 200
    ids = {row["campaign_talent_id"] for row in response .json()}
...
def test_release_stuck_payment_requires_stripe_onboarding(client, db, admin_headers):
    ...
    now = datetime.now(timezone.utc)
    _seed_campaign_rep(
        db, campaign_id, talent_id, processing_started_at=now - timedelta(hours=49), stripe_transfer_id="tr_stuck2"
    )

    response  = client.post("/admin/payments/tr_stuck2/release", headers=admin_headers)
    # talent never completed Stripe Connect onboarding in this test -- the
    # audit-flagged release still correctly refuses to transfer money
    # nowhere.
    assert response .status_code == 409
    assert response .json()["error"]["code"] == "talent_not_onboarded"
...
def test_analytics_reps_by_city_and_category(client, db, admin_headers):
    _seed_rep(db)
    response  = client.get("/admin/analytics/talents", headers=admin_headers)
    assert response .status_code == 200
    body = response .json()
...
def test_outlier_brand_all_five_star_flagged(client, db, admin_headers):
    ...
    for _ in range(3):
        _, talent_id = _seed_rep(db)
        cr_id = _seed_campaign_rep(db, campaign_id, talent_id, status="paid", payout_status="paid")
        db.execute("UPDATE public.campaign_talents SET brand_rating = 5 WHERE id = $1", cr_id)
...
def test_parent_suspension_queue_and_reversal(client, db, admin_headers, seed_talent_with_parent):
    ...
    assert any(row["talent_id"] == seeded.talent_id for row in queue_response.json())

    reverse_response = client.post(f"/admin/parent-suspensions/{seeded.talent_id}/reverse", headers=admin_headers)
    ...
    assert not any(row["talent_id"] == seeded.talent_id for row in still_queued.json())
...
def test_admin_only_reverses_parent_initiated_suspension(client, db, admin_headers):
    ...
    talent_user_id, talent_id = _seed_rep(db)
    db.execute("UPDATE public.users SET account_status = 'suspended' WHERE id = $1", talent_user_id)

    response  = client.post(f"/admin/parent-suspensions/{talent_id}/reverse", headers=admin_headers)
    assert response .status_code == 404
...
def test_talent_can_file_safety_report_and_admin_resolves_it(client, db, auth_headers_factory, admin_headers):
    talent_user_id, talent_id = _seed_rep(db)
    import time
...
def test_safety_report_resolve_twice_is_409(client, db, admin_headers):
    talent_user_id, talent_id = _seed_rep(db)
    db.execute(
        "INSERT INTO public.safety_reports (id, reporter_talent_id, reason) VALUES ($1, $2, 'test')",
