...
def test_request_link_known_email_sends_magic_link(client, fake_resend_client, seed_rep_with_parent):
    seeded = seed_rep_with_parent(parent_email="knownparent@example.com")
...
def test_request_link_rate_limited_does_not_reveal_via_status_code(client, fake_resend_client, seed_rep_with_parent):
    seeded = seed_rep_with_parent(parent_email="rl-parent@example.com")
...
def test_verify_valid_token_issues_session(client, fake_resend_client, seed_rep_with_parent):
    seeded = seed_rep_with_parent(parent_email="verify-ok@example.com")
...
def test_verify_token_used_twice_fails_second_time(client, fake_resend_client, seed_rep_with_parent):
    seeded = seed_rep_with_parent(parent_email="verify-reuse@example.com")
    client.post("/parent/auth/request-link", json={"parent_email": seeded.parent_email})
    token = _extract_magic_link_token(fake_resend_client.sent[0].html)
...
def test_verify_after_portal_expired_returns_portal_closed(client, db, seed_rep_with_parent):
    seeded = seed_rep_with_parent(age=18, portal_expires_in_days=-1)
...
def test_expired_portal_session_rejected_on_every_request(client, parent_headers_factory, seed_rep_with_parent):
    seeded = seed_rep_with_parent(age=19, portal_expires_in_days=-30)
    headers = parent_headers_factory(parent_id=seeded.parent_id, rep_id=seeded.rep_id)
...
def test_dashboard_returns_rep_summary(client, parent_headers_factory, seed_rep_with_parent):
...
def test_pending_campaigns_returns_full_brief(client, parent_headers_factory, seed_rep_with_parent, seed_pending_campaign):
    seeded = seed_rep_with_parent()
    seed_pending_campaign(rep_id=seeded.rep_id)
    headers = parent_headers_factory(parent_id=seeded.parent_id, rep_id=seeded.rep_id)
    ...
    assert brief["payout_per_rep_cents"] == 5000
    assert brief["deliverables_description"] == "One TikTok post"


def test_approve_campaign_is_idempotent(client, db, parent_headers_factory, seed_rep_with_parent, seed_pending_campaign):
    seeded = seed_rep_with_parent()
    campaign_id = seed_pending_campaign(rep_id=seeded.rep_id)
    headers = parent_headers_factory(parent_id=seeded.parent_id, rep_id=seeded.rep_id)
    ...
    rows = db.fetch("SELECT parent_approval_status FROM public.campaign_reps WHERE campaign_id = $1", campaign_id)
    assert rows[0]["parent_approval_status"] == "approved"


def test_block_campaign_auto_declines_without_exposing_reason_to_brand(
    client, db, fake_resend_client, parent_headers_factory, seed_rep_with_parent, seed_pending_campaign
):
    seeded = seed_rep_with_parent()
    campaign_id = seed_pending_campaign(rep_id=seeded.rep_id)
    headers = parent_headers_factory(parent_id=seeded.parent_id, rep_id=seeded.rep_id)
    ...
    rows = db.fetch(
        "SELECT status, parent_approval_status FROM public.campaign_reps WHERE campaign_id = $1", campaign_id
    )
    # 'declined' is exactly what a rep's own decline produces -- nothing
    # brand-visible distinguishes a parent block from a rep decline.
    assert rows[0]["status"] == "declined"
    assert rows[0]["parent_approval_status"] == "blocked"

    # The rep is told a campaign was declined on their behalf (that's
    # fine -- they know their own parent is involved). NOTE: the
    # brand-facing side of this ("neutral message to the brand") can't
    # be exercised end-to-end yet since GET /brands/campaigns/:id/reps
    # doesn't exist until Prompt 8; the 'declined' status assertion
    # above -- the same value a rep's own decline produces, with
    # nothing brand-visible distinguishing the two -- is the
    # enforceable part today.
    assert len(fake_resend_client.sent) == 1


def test_approve_unknown_campaign_returns_404(client, parent_headers_factory, seed_rep_with_parent):
    seeded = seed_rep_with_parent()
    headers = parent_headers_factory(parent_id=seeded.parent_id, rep_id=seeded.rep_id)
...
def test_get_settings_returns_current_state(client, parent_headers_factory, seed_rep_with_parent):
    seeded = seed_rep_with_parent(values_filters=["alcohol_adjacent"], campaign_approval_required=True, digest_enabled=False)
    headers = parent_headers_factory(parent_id=seeded.parent_id, rep_id=seeded.rep_id)
...
def test_update_values_filters_accepts_valid_categories(client, parent_headers_factory, seed_rep_with_parent):
    seeded = seed_rep_with_parent()
    headers = parent_headers_factory(parent_id=seeded.parent_id, rep_id=seeded.rep_id)
...
def test_update_values_filters_rejects_unknown_category(client, parent_headers_factory, seed_rep_with_parent):
    seeded = seed_rep_with_parent()
    headers = parent_headers_factory(parent_id=seeded.parent_id, rep_id=seeded.rep_id)
...
def test_approval_required_toggle_locked_under_16(client, parent_headers_factory, seed_rep_with_parent):
    seeded = seed_rep_with_parent(age=15)
    headers = parent_headers_factory(parent_id=seeded.parent_id, rep_id=seeded.rep_id)
...
def test_approval_required_toggle_allowed_at_16(client, parent_headers_factory, seed_rep_with_parent):
    seeded = seed_rep_with_parent(age=16)
    headers = parent_headers_factory(parent_id=seeded.parent_id, rep_id=seeded.rep_id)
...
def test_digest_setting_toggle(client, parent_headers_factory, seed_rep_with_parent):
    seeded = seed_rep_with_parent(digest_enabled=True)
    headers = parent_headers_factory(parent_id=seeded.parent_id, rep_id=seeded.rep_id)
...
def test_digest_preview_first_time_has_no_completeness_change(client, parent_headers_factory, seed_rep_with_parent):
    seeded = seed_rep_with_parent(profile_completeness_score=70, total_earnings_cents=5000)
    headers = parent_headers_factory(parent_id=seeded.parent_id, rep_id=seeded.rep_id)
...
def test_monthly_digest_job_excludes_recruiter_and_submission_content(
    client, db, fake_resend_client, seed_rep_with_parent, monkeypatch
):
    seed_rep_with_parent(digest_enabled=True)
...
def test_suspend_account_sets_status_and_notifies_rep(client, db, fake_resend_client, parent_headers_factory, seed_rep_with_parent):
    seeded = seed_rep_with_parent()
    headers = parent_headers_factory(parent_id=seeded.parent_id, rep_id=seeded.rep_id)
...
def test_unsuspend_reverses_parent_initiated_suspension(client, db, parent_headers_factory, seed_rep_with_parent):
...
def test_unsuspend_rejects_admin_initiated_suspension(client, parent_headers_factory, seed_rep_with_parent):
    seeded = seed_rep_with_parent(rep_account_status="suspended", suspended_by_parent_at=None)
    headers = parent_headers_factory(parent_id=seeded.parent_id, rep_id=seeded.rep_id)
