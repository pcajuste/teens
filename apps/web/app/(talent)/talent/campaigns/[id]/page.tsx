...
import { MilestoneList } from "@/components/talent/milestone-list";
...
export default function CampaignDetailPage() {
  ...
  async function load() {
    try {
      const [available, active, history] = await Promise.all([
        ...
        api.get<CampaignParticipation[]>("/talents/campaigns/history"),
      ]);
      const found = available.find((c) => c.id === campaignId) ?? null;
      setCampaign(found);
      const cr =
        [...active, ...history].find((p) => p.campaign_id === campaignId) ??
        null;
      setParticipation(cr);
  ...
  async function handleAccept() {
    ...
    try {
      ...
      await api.post(`/campaigns/${campaignId}/accept`, {
        ftc_disclosure_accepted: ftcAccepted,
      });
      trackEvent("campaign_accepted", {
      ...
      if (err instanceof ApiError && err.code === "ftc_module_required") {
        ...
        trackEvent("ftc_gate_triggered", {
          module_id: (err.detail?.module_id as string) ?? null,
        });
  ...
  return (
    <TalentShell backHref="/talent">
      <div className="flex flex-col gap-5">
        ...
        participation.milestones.length > 0 ? (
          <section className="flex flex-col gap-3">
            ...
            <MilestoneList
              ...
              payoutPerRepCents={campaign?.payout_per_talent_cents ?? null}
              onChanged={load}
            />
          </section>
        ) : null}
        ...
        (participation.status === "accepted" ||
          participation.status === "revision_requested") ? (
          <section className="flex flex-col gap-3 rounded-xl border border-border bg-card p-5">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <p className="text-xs text-muted-foreground">
                JPEG, PNG, WEBP, HEIC, MP4, or MOV. Up to 25MB each.
              </p>
            ...
            {!participation.ftc_disclosure_accepted ? (
              <p className="text-xs text-destructive">
                You must accept the FTC sponsorship disclosure before
                submitting.
              </p>
            ) : null}
          </section>
        ...
        {participation &&
        participation.payment_type !== "milestone" &&
        ["submitted", "under_review", "confirmed", "paid"].includes(
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-sm rounded-xl bg-card p-6 shadow-lg">
            <p className="text-lg font-semibold">
              Complete the FTC module first
            </p>
            <p className="mt-2 text-sm text-muted-foreground">
              Before accepting campaigns, complete the FTC Disclosure Essentials
              module. It takes about 5 minutes and ensures you understand the
              sponsored content disclosure rules that protect you and your
              followers.
            </p>
            <div className="mt-4 flex flex-col gap-2">
              <Link
                href={
                  ftcGateModuleId
                    ? `/talent/learning/${ftcGateModuleId}`
                    : "/talent/learning"
                }
                className="w-full"
              >
              ...
              <Button
                variant="ghost"
                size="sm"
                onClick={() => setFtcGateModuleId(null)}
              >
                Not now
              </Button>
            </div>
