...
export default function OnboardingPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setPending(true);
    setError(null);
    const body: TalentProfileUpdateRequest = {
    ...
    try {
      await api.put<TalentProfile>("/talents/me", body);
      // recruiter_visible is not writable via PUT /talents/me in the
      // current backend schema (Prompt 5 deliverable scope) -- the
      // toggle below records the talent's intent locally for now.
      router.push("/talent");
    } catch (err) {
      setError(
    ...
    <main className="mx-auto flex min-h-screen max-w-md flex-col gap-6 p-4 pb-16">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          This is what brands and recruiters see when deciding whether to work
          with you.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <p className="text-xs text-muted-foreground">
            Only used in anonymized, aggregated trend reports -- never shown on
          ...
          <div className="flex flex-wrap gap-2">
            {SCHOOL_TYPES.map((t) => (
              <button
                ...
                type="button"
                onClick={() =>
                  setSchoolType(schoolType === t.value ? null : t.value)
                }
                className="min-h-11"
              >
                <Badge
                  variant={schoolType === t.value ? "default" : "outline"}
                  className="px-3 py-2"
                >
                  {t.label}
                </Badge>
        ...
        <p className="-mt-2 text-xs text-muted-foreground">
          Display-only -- Teenure never embeds or auto-imports your social
          content.
        </p>
        ...
        <div className="flex items-start gap-3 rounded-lg border border-border p-3">
          ...
          >
            ...
            <span className="text-xs text-muted-foreground">
              Off by default. Turning this on lets verified college and employer
              recruiters find and view your profile. You can turn it off again
              at any time.
            </span>
          </Label>
        </div>
