...
// Fetches GET /talents/me/achievement-record, which wraps the exact same
// data as GET /talents/me/profile-preview (see AchievementRecordResponse
// in apps/api/app/schemas/talents.py) -- this page just renders it as a
// clean, printable document. There is no server-side PDF generation:
// the talent uses their browser's native "Print > Save as PDF", which is
...
export default function AchievementRecordPage() {
  ...
  useEffect(() => {
    api
      .get<AchievementRecord>("/talents/me/achievement-record")
      .then(setData)
      .catch((err) =>
        setError(
          err instanceof ApiError
            ? err.message
            : "Could not load achievement record.",
        ),
      );
  }, []);
        ...
        <article className="flex flex-col gap-5 rounded-lg border border-border p-6 print:border-0 print:p-0">
          <header className="border-b border-border pb-4">
            ...
            <p className="mt-2 text-xs text-muted-foreground">
              ...
              every entry below is confirmed by the brand that ran the campaign,
              not self-reported.
            </p>
          </header>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="rounded-lg border border-border p-3 print:border">
              ...
              <p className="text-lg font-semibold">
                {record.total_campaigns_completed}
              </p>
            </div>
            <div className="rounded-lg border border-border p-3 print:border">
              <p className="text-xs text-muted-foreground">
          ...
          <footer className="border-t border-border pt-3 text-xs text-muted-foreground">
            Verified via teenure.com. This record reflects only brand-confirmed
            campaigns -- no self-reported claims, no public posts.
          </footer>
        </article>
      ) : null}
    </main>
