...
import type { InsightBrandResult, InsightCampaign, InsightEligibility } from "@/lib/types";
...
export default function BrandInsightFeedbackPage() {
  ...
  const [results, setResults] = useState<InsightBrandResult[] | null>(null);
  ...
  async function viewResults(id: string) {
    setResultsFor(id);
    const rows = await api.get<InsightBrandResult[]>(`/brands/insight/campaigns/${id}/results`);
    setResults(rows);
  }
