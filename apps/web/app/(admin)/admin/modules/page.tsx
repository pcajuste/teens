 ...
 * exactly what a talent sees -- correct answers hidden client-side too,
...
export default function AdminModulesPage() {
  ...
  async function load() {
    try {
      ...
      setError(
        err instanceof ApiError ? err.message : "Could not load modules.",
      );
    }
  }
  ...
  function addBlock(type: ContentBlockType) {
    setBlocks((prev) => [
      ...prev,
      type === "quiz"
        ? { type, text: "", questions: [emptyQuestion()] }
        : { type, text: "", questions: [] },
    ]);
  }
  ...
  async function handleCreate() {
    ...
    try {
      ...
      setError(
        err instanceof ApiError ? err.message : "Could not create module.",
      );
    } finally {
      setPending(false);
    }
  ...
  async function handleActivate(id: string) {
    try {
      ...
      setError(
        err instanceof ApiError ? err.message : "Could not activate module.",
      );
    }
  }
  ...
  async function handleArchive(id: string) {
    try {
      ...
      setError(
        err instanceof ApiError ? err.message : "Could not archive module.",
      );
    }
  }
    ...
    >
      ...
      {showBuilder ? (
        <Card>
          <CardHeader>
            <CardTitle>
              {preview
                ? "Preview (talent view — correct answers hidden)"
                : "Build a module"}
            </CardTitle>
          </CardHeader>
          <CardContent>
              ...
              <div className="flex flex-col gap-4">
                    ...
                    >
                      ...
                      {block.type !== "quiz" ? (
                        <Textarea
                          ...
                          placeholder={
                            block.type === "text" ? "Body text…" : "https://…"
                          }
                        />
                      ) : (
                        <div className="flex flex-col gap-3">
                          {block.questions.map((q, qi) => (
                            ...
                            >
                              ...
                              {q.options.map((opt, oi) => (
                                <label
                                  key={oi}
                                  className="mb-1 flex min-h-9 items-center gap-2 text-sm"
                                >
                                  <input
                                    type="radio"
                                    name={`correct-${i}-${qi}`}
                ...
                <div className="flex gap-2">
                  ...
                  <Button
                    ...
                    disabled={
                      pending || !title || !description || blocks.length === 0
                    }
                  >
                      ...
                      ? block.questions.map((q, qi) => (
                          ...
                          >
                            ...
                            {/* correct_index deliberately never rendered here -- admin
                               preview shows exactly what a talent sees. */}
                          </div>
                        ))
                      : null}
                  </div>
      ...
      {modules === null ? (
        ...
        <EmptyState
          ...
          description="Create the first module to open the Learning Hub."
        />
      ) : (
        <div className="flex flex-col gap-3">
          {modules.map((m) => (
            ...
            <Card key={m.id} className={m.status === "archived" ? "opacity-60" : undefined}>
              <CardContent className="flex flex-col gap-2 p-4">
                ...
                <div className="flex flex-wrap gap-3 text-xs text-text-2">
                  ...
                  <span>
                    {m.pass_rate !== null
                      ? `${Math.round(m.pass_rate * 100)}% pass rate`
                      : "no completions yet"}
                  </span>
                  <span>
                    {m.average_attempts !== null
                      ? `${m.average_attempts} avg attempts`
                      : ""}
                  </span>
                  {m.in_progress_count > 0 ? (
                    <span>{m.in_progress_count} in progress</span>
                  ) : null}
                </div>
              </CardContent>
            </Card>
