...
export default function DemoCampaignDetailPage({
  params,
}: {
  params: { id: string };
}) {
  ...
  if (id === DEMO_AVAILABLE_CAMPAIGN_ID) {
    return (
      <main className="mx-auto flex min-h-screen max-w-md flex-col gap-5 p-4 pb-16">
        <DemoPageViewTracker demo="talent_campaign_detail" />
        <DemoBanner />
        <DemoBackLink href="/demo/talent" label="Back to dashboard" />

        <div>
          <h1 className="text-xl font-semibold">
            {DEMO_AVAILABLE_CAMPAIGN.title}
          </h1>
        ...
        <section className="flex flex-col gap-2 rounded-lg border border-border p-3">
          ...
          <div>
            ...
            <p className="text-sm">
              {DEMO_AVAILABLE_CAMPAIGN.deliverables_description}
            </p>
          </div>
        ...
        <section className="rounded-lg border border-border bg-muted/40 p-3 text-sm text-muted-foreground">
          In the real app, you&apos;d apply here, accept the FTC sponsorship
          disclosure, and submit your work once approved. This demo is
          read-only, so applying is disabled.
        </section>
      ...
      <main className="mx-auto flex min-h-screen max-w-md flex-col gap-5 p-4 pb-16">
        ...
        <section className="flex flex-col gap-2 rounded-lg border border-border p-3">
          ...
          <div>
            ...
            <p className="text-sm">
              {DEMO_CONFIRMED_CAMPAIGN.deliverables_description}
            </p>
          </div>
          <div className="flex items-center justify-between pt-1">
            <p className="text-xs font-medium text-muted-foreground">Payout</p>
        ...
        <Card>
          ...
          <CardContent className="flex flex-col gap-2">
            <p className="text-sm">
              {DEMO_CONFIRMED_PARTICIPATION.submission_text}
            </p>
        ...
        <section className="rounded-lg border border-border bg-muted/40 p-3 text-sm text-muted-foreground">
          This campaign is shown as confirmed and awaiting payout. This demo is
          read-only, so there&apos;s no withdraw action here.
        </section>
