    ...
    <main className="mx-auto flex min-h-screen max-w-md flex-col gap-6 p-4 pb-16">
      ...
      <header>
        ...
        <p className="text-sm text-muted-foreground">
          A preview of what a Teenure rep dashboard looks like once your profile is fully built out.
        </p>
      </header>

      <section>
        <ProfileView profile={DEMO_REP_PROFILE} />
      </section>

      <section>
        <CompletenessPanel profile={DEMO_REP_PROFILE} />
      </section>
      ...
      <section className="flex flex-col gap-2">
        <h2 className="text-sm font-semibold text-muted-foreground">Active campaign</h2>
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between gap-2">
              <CardTitle>{DEMO_CONFIRMED_CAMPAIGN.title}</CardTitle>
              <Badge variant="secondary">{STATUS_LABEL[DEMO_CONFIRMED_PARTICIPATION.status] ?? DEMO_CONFIRMED_PARTICIPATION.status}</Badge>
            </div>
          </CardHeader>
          <CardContent>
            ...
            <div className="flex items-center justify-between pt-2">
              <Link
                href={`/demo/rep/campaigns/${DEMO_CONFIRMED_CAMPAIGN.id}`}
                className="text-sm font-medium underline"
              >
                View details
              </Link>
              <span className="text-sm font-semibold">{money(DEMO_CONFIRMED_PARTICIPATION.payout_cents)}</span>
            </div>
          </CardContent>
        </Card>
      </section>
      ...
      <section className="flex flex-col gap-2">
        <h2 className="text-sm font-semibold text-muted-foreground">Available campaign</h2>
        <Link href={`/demo/rep/campaigns/${DEMO_AVAILABLE_CAMPAIGN.id}`} className="block">
          <Card className="min-h-11">
            ...
            <CardContent>
              ...
              <div className="flex items-center justify-between pt-1">
                <span className="text-sm font-semibold">{money(DEMO_AVAILABLE_CAMPAIGN.payout_per_rep_cents)}</span>
                <div className="flex flex-wrap gap-1">
                  {DEMO_AVAILABLE_CAMPAIGN.target_categories.slice(0, 3).map((c) => (
