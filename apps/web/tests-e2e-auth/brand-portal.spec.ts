...
// talent-signup-and-login.spec.ts's shape for the brand side.
//
...
//
// Does not cover talent browse/invite (needs a seeded talent with
...
async function reloginToPickUpApproval(
  page: import("@playwright/test").Page,
  email: string,
) {
...
test.describe("brand signup and onboarding", () => {
  test("creates a real account, completes company profile, stays gated pending approval", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByRole("link", { name: "Finish your company profile" }),
    ).toBeVisible();
  });
});

test.describe("campaign creation and activation", () => {
  test("brief builder shows a live preview and creates a real campaign", async ({
    page,
  }) => {
    ...
    await page.getByLabel("Max talents").fill("5");
    ...
    await expect(
      page.getByRole("heading", { name: "Spring Launch" }),
    ).toBeVisible();
    await expect(page.getByText("Draft")).toBeVisible();
  });

  test("activating a draft campaign creates a real Stripe PaymentIntent and transitions status", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByText(/Payment initiated|Something went wrong|error/i).first(),
    ).toBeVisible({
      timeout: 15_000,
    });
  });
