...
// demo-talent-portal.spec.ts, this suite genuinely creates auth.users
...
test.describe("Talent signup", () => {
  ...
  test("rejects a duplicate email with a real backend error, not a client-side guess", async ({
    page,
  }) => {
...
test.describe("Talent login", () => {
  test("an existing account can sign in and reach the dashboard", async ({
    page,
  }) => {
  ...
  test("wrong password is rejected with a real GoTrue error", async ({
    page,
  }) => {
