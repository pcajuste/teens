...
/** Discovery panel on the talent dashboard (Build Prompt 8G frontend
 * deliverable). Visually distinct from the Campaigns panel -- a
 * different header treatment and an explicit "unpaid" label -- so a
 * talent never confuses a challenge for a guaranteed-pay campaign. */
    ...
    <section className="flex flex-col gap-3 rounded-xl border border-dashed border-border bg-card/60 p-4">
      <div>
        <h2 className="text-sm font-semibold text-muted-foreground">
          Brand Challenges — Unpaid Discovery
        </h2>
        <p className="text-xs text-muted-foreground">
          Submit your creative work. Brands may invite you to a paid campaign
      ...
      {challenges.length === 0 ? (
        <EmptyState
          ...
          description="Check back soon for new brand challenges."
        />
      ) : (
              ...
              <div>
                ...
                {s.status === "converted" ? (
                  <p className="text-sm text-muted-foreground">
                    Brand invited you to {s.campaign_title ?? "a campaign"}. +
                    {money(s.bonus_cents)} bonus added to your earnings.
                  </p>
                ) : (
                  <p className="text-sm text-muted-foreground">
                    Submitted — brand is reviewing
                  </p>
                )}
              </div>
              <Badge
                variant={s.status === "converted" ? "success" : "secondary"}
              >
