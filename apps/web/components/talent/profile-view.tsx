...
import type { TalentProfile, TalentProfilePreview } from "@/lib/types";

function money(cents: number): string {
  return `$${(cents / 100).toFixed(2)}`;
}

// Renders identically whether fed GET /talents/me or GET
// /talents/me/profile-preview data, so the "what a recruiter sees" screen
...
export function ProfileView({
  ...
  const earnings =
    "total_earnings_cents" in profile ? profile.total_earnings_cents : null;
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {/* Verified badges (Build Prompt 8H) -- rendered identically here
         whether this is the talent's own /talents/me view or the
         profile-preview view brands/recruiters see, per the spec's
         "same rendering ... so talents see exactly what brands and
      ...
      <div className="flex flex-wrap gap-3 text-sm text-muted-foreground">
        {profile.instagram_handle ? (
          <span>Instagram: @{profile.instagram_handle}</span>
        ) : null}
        {profile.tiktok_handle ? (
          <span>TikTok: @{profile.tiktok_handle}</span>
        ) : null}
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 pt-1">
        <div className="rounded-lg border border-border p-2.5">
          ...
          <p className="text-lg font-semibold">
            {profile.total_campaigns_completed}
          </p>
        </div>
        <div className="rounded-lg border border-border p-2.5">
