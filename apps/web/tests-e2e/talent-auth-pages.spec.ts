...
// Smoke coverage for the talent signup page (still role-specific) and the
// shared /login page. Every other /talent/* route is gated behind AuthGate
...
// portal at /demo/talent, covered in demo-talent-portal.spec.ts, has no such
...
test.describe("/talent/signup", () => {
  test("loads and renders the signup form without a client-side exception", async ({
    page,
  }) => {
...
test.describe("/login", () => {
  test("loads and renders the login form without a client-side exception", async ({
    page,
  }) => {
    const { errors, consoleErrors } = collectPageErrors(page);
    ...
    await expect(
      page.getByRole("heading", { name: "Sign in to Teenure" }),
    ).toBeVisible();
    await expect(page.getByLabel("Email", { exact: true })).toBeVisible();
    await expect(page.getByLabel("Password", { exact: true })).toBeVisible();
...
test.describe("gated /talent routes", () => {
