...
// /demo/talent (Build Prompt 6A). Unlike the real (talent) portal, this route
...
const DEMO_talent_NAME = "Maya Chen";
...
test.describe("/demo/talent dashboard", () => {
  test("renders demo banner, profile, campaigns, and CTA with no backend calls", async ({
    page,
    baseURL,
  }) => {
    ...
    await page.goto("/demo/talent");

    await expect(page.getByText(/Demo — this is example data/i)).toBeVisible();
    await expect(
      page.getByRole("heading", { name: DEMO_talent_NAME }),
    ).toBeVisible();
    ...
    await expect(cta).toHaveAttribute("href", "/talent/signup");
    ...
    expect(
      foreignRequests,
      `Unexpected non-local requests: ${foreignRequests.join("; ")}`,
    ).toEqual([]);
  });

  test("CTA navigates into the real signup flow, not a demo shortcut", async ({
    page,
  }) => {
    await page.goto("/demo/talent");
    ...
    await expect(
      page.getByRole("heading", { name: "Create your Teenure account" }),
    ).toBeVisible();
  });
});

test.describe("/demo/talent/campaigns/[id] -- available campaign", () => {
  test("renders campaign detail with no apply/accept controls", async ({
    page,
    baseURL,
  }) => {
    ...
    await page
      .getByRole("link")
      .filter({ hasText: AVAILABLE_CAMPAIGN_TITLE })
      .click();
    await expect(page).toHaveURL(
      new RegExp(`/demo/talent/campaigns/${AVAILABLE_CAMPAIGN_ID}$`),
    );

    await expect(
      page.getByRole("heading", { name: AVAILABLE_CAMPAIGN_TITLE }),
    ).toBeVisible();
    ...
    expect(
      foreign,
      `Unexpected non-local requests: ${foreign.join("; ")}`,
    ).toEqual([]);
  });
});

test.describe("/demo/talent/campaigns/[id] -- confirmed campaign", () => {
  test("renders submission evidence and status tracker with no withdraw action", async ({
    page,
    baseURL,
  }) => {
    const foreign = collectForeignRequests(page, baseURL!);

    await page.goto(`/demo/talent/campaigns/${CONFIRMED_CAMPAIGN_ID}`);

    await expect(
      page.getByRole("heading", { name: CONFIRMED_CAMPAIGN_TITLE }),
    ).toBeVisible();
    await expect(page.getByText(/submitted work/i)).toBeVisible();
    await expect(
      page.getByText(/mock file, not a real upload/i).first(),
    ).toBeVisible();
