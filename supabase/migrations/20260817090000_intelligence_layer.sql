...
--     There is no campaign_id, talent_id, brand_id, campaign_talent_id, or
--     user_id column at all -- not even an unconstrained one -- so no
--     join back to talent_profiles/users/campaign_talents/campaigns can ever
...
--     to a single identifiable person the way e.g. talent_id + exact
--     payout_cents + exact timestamp could.
--   - school_type is nullable at the source (talent_profiles.school_type,
...
--
-- talent_profiles has no location field finer than city/state (see
...
-- same talent_profiles.city/state values; there is nothing more granular
...
CREATE TABLE public.intelligence_events_anonymized (
  ...
  -- trend reports" cadence. Derived from the campaign_talents transition
  ...
  status              talent_campaign_status NOT NULL CHECK (status IN ('confirmed','paid')),
...
-- every Postgres role subject to RLS (anon/authenticated), so talent,
...
-- ──────────────────────────────────────────────────────────────────
-- Write-path bookkeeping: which campaign_talents rows have already been
...
-- column lives on campaign_talents (the source-of-truth table, already
...
CREATE INDEX idx_campaign_reps_intelligence_pending
  ON public.campaign_talents (status)
