...
- **Not a dating platform.** No matching between users. No "who viewed my profile" signals between talents. No talent-to-talent contact of any kind.
- **Not Instagram or TikTok.** Talents do not post content on Teenure. They submit campaign evidence to brands. Those submissions are private — visible only to the brand and the talent, never to other talents or the public.
...
- **Not an influencer marketplace.** Talents are not influencers. They are verified peer voices with documented real-world track records.
...
If a proposed feature does not directly contribute to building, verifying, or surfacing a talent's achievement record — it does not get built. This rule is non-negotiable and applies to every product decision.
...
| -------------------------------- | --------------------------- | --------------------------------- |
| Profile bio                      | Talent profile              | Brands + opted-in recruiters only |
| Category selections              | Talent profile              | Brands + opted-in recruiters only |
| Campaign submission text         | Campaign record             | Brand + Talent only               |
| Campaign submission file uploads | Campaign record             | Brand + Talent only               |
| Brand rating and rating note     | Talent profile (aggregated) | Brands + opted-in recruiters only |
| Recruiter message to Talent      | Talent inbox                | Talent only                       |
| Talent reply to recruiter        | Recruiter thread            | Recruiter only                    |

### What Is Prohibited — With Technical Enforcement

| Prohibited                                      | Enforcement Mechanism                                                                                                                                                                                                                           |
| ----------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Talent-to-talent messaging                            | No messaging UI exists between talents. No endpoint exists. Not a missing feature — a deliberate absence.                                                                                                                                        |
| Talent-to-brand unsolicited contact                | Talents cannot initiate contact with brands. Talents only respond to campaign invitations.                                                                                                                                                            |
| Talent-to-recruiter contact (unsolicited or reply) | Talents can only read messages a recruiter sends. No reply field, endpoint, or UI exists on the Talent side. `recruiter_contacts` has no talent-authored column — the schema itself makes a reply structurally unstorable, not just hidden by the UI. |
...
| Profile browsing between talents                | Talents cannot search, view, or discover other Talent profiles. The Talent search interface is available only to authenticated brand and recruiter accounts.                                                                                       |
...
- **To talents:** Your profile here is taken seriously because nothing here isn't serious.
...
### 2.1 The Talent (Teen, Ages 14–18)
...
- Engagement performance per campaign (submitted by talent, confirmed by brand)
...
Pays to access the Talent network for authentic teen peer-influence campaigns. Also pays for aggregated, anonymized trend intelligence derived from campaign performance data.
...
1. **Campaign Access** — pay per campaign to activate a curated set of talents
...
### 2.4 The Parent (Guardian of a Minor Talent — Oversight Role, Not a Platform Side)

Teenure remains a three-sided platform (Talent, Brand, Recruiter) — the parent is not a fourth revenue-generating side, and does not have an `auth.users` account or a `user_role` value. It is a scoped, non-paying oversight role that exists because Teenure handles minors' data and income: a `parent_records` row links a parent to their (under-18) Talent and grants a deliberately narrow set of permissions. See [Section 9A](#9a-parent-portal) for the full spec.

**Core motivations:**

- See what their child is doing on the platform without needing the talent's own login
...
**What this role explicitly does not have:** co-pilot access to the talent's account (cannot message recruiters, cannot edit the talent's profile, cannot submit campaign work), access to recruiter message content or campaign submission files, or any role beyond age 18 (the portal expires automatically at the talent's 18th birthday).
...
Brand submits brief
        ↓
Platform surfaces matching talents (by category, city, history)
        ↓
Brand selects talents OR platform auto-assigns by fit score
        ↓
Talent receives brief → accepts or declines within 48 hours
        ↓
Talent completes deliverables (posts, events, word-of-mouth, reviews)
        ↓
Talent submits evidence (screenshots, links, written notes)
        ↓
Brand reviews evidence → confirms completion OR requests revision
        ↓
Brand submits rating (1–5 stars + optional note)
        ↓
Stripe Transfer releases payout to talent's Connected Account
        ↓
...
A Talent with a two-year verified profile **cannot rebuild it on a competitor platform**. Every campaign makes the profile more valuable. Moving means starting from zero.

Three compounding network effects:

| Effect                  | Mechanism                                                                                                                       |
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| **Geographic density**  | 20 talents in one city is sellable. Thin national spread is not. Brands pay for concentrated local reach.                       |
| **Category clustering** | 50 verified athletes is worth more than 500 random teens. Clusters drive brand willingness to pay.                              |
| **Data flywheel**       | More campaigns → richer intelligence → higher brand willingness to pay → more budget → more talents attracted → more campaigns. |

### 3.4 The College Recruitment Mechanic

Talents opt in to recruiter visibility. Every campaign they complete is simultaneously **income and college application material**. This inverts the incentive entirely:
...
Colleges pay for search access. Talents want to be found. Every additional recruiter makes every Talent profile more worth completing. Every additional Talent makes the recruiter product more useful.
...
| Item           | Detail                                                                             |
| -------------- | ---------------------------------------------------------------------------------- |
| Pricing        | $500–$5,000 per campaign depending on Talent count and category specificity        |
| Platform take  | 30–40% of campaign value. Remainder distributed to talents                         |
...
| Period     | Target                                                                           |
| ---------- | -------------------------------------------------------------------------------- |
| Month 1–3  | $0 revenue. Build Talent network manually. Prove the model.                      |
...
### Phase 1 — Talent Portal (Build First)
...
- Available campaigns panel: open campaigns matching talent's categories
...
- Part of Phase 1, not a later phase: minors' campaign participation is gated on it (parent approval, values filters), so it must exist before Phase 1 is considered "fully functional" for under-18 talents.
...
- One-tap campaign withdrawal (no penalty, no reason required) is a Talent Portal feature in the same spirit — surfaced prominently, not buried in settings.
...
- Brief builder with fields:
  ...
  - Target Talent categories (multi-select)
  - Target cities (multi-select)
  - Max number of talents
- Preview before submission
- Stripe payment capture at campaign activation

#### Talent Discovery

- Browse opted-in Talent profiles filtered by category, city, graduation year, campaign history
- Talent card: categories, campaigns completed, average rating, city — no PII at browse stage
- Full profile view on click (no contact credit required for brands)
- Invite specific talents to campaign
- OR: request platform auto-match based on category and city targeting

#### Campaign Management

- Campaign dashboard: all campaigns with status
- Per-campaign view: Talent list with individual statuses
- Submission review: view Talent evidence, approve or request revision with note
- Completion confirmation: single action triggers Stripe payout
- Rating submission: 1–5 stars + optional written note per talent
...
- Talent result cards: category badges, campaign count, graduation year, city — no PII until contact credit spent
- Full profile view costs one contact credit
- Saved profiles list: build and manage prospect lists

#### Messaging

- Direct message to Talent costs one contact credit
- Message delivered to talent's Teenure inbox (not their personal email at MVP)
- Talent receives in-app notification
...
- Talent approval queue: review new signups, approve or reject
...
| Payments        | Stripe + Stripe Connect                 | Campaign billing + Talent payouts in one integration       |
...
CREATE TYPE user_role AS ENUM ('talent', 'brand', 'recruiter', 'admin');
...
CREATE TYPE talent_campaign_status AS ENUM (
...
-- not_required: talent is 18+, or talent is 16-17 with campaign_approval_required = FALSE.
-- pending: campaign_approval_required = TRUE for this talent and the linked parent
...
--   GET /talents/campaigns/available in the first place, so it never gets a
...
CREATE TABLE public.talent_profiles (
  ...
  school_type                 TEXT,                            -- 'public' | 'private' | 'charter' | 'homeschool'; nullable at MVP (self-reported, optional at onboarding), used only in aggregate by the intelligence layer — never joined back to an individual talent in any report
  city                        TEXT NOT NULL,
  state                       TEXT NOT NULL,
...
CREATE TABLE public.campaigns (
  ...
  payout_per_talent_cents      INTEGER,                  -- talent_pool_cents / max_reps
...
-- ──────────────────────────────────────────────────────────────────
-- CAMPAIGN TALENTS (join table — one row per talent per campaign)
-- ──────────────────────────────────────────────────────────────────
...
CREATE TABLE public.parent_records (
  ...
  -- used by talent_profiles.categories / campaigns.target_categories, plus
...
CREATE INDEX idx_campaign_reps_rep ON public.campaign_reps (talent_id);
...
-- Talents see only their own profile in edit mode
...
-- Brands see talents only in campaign context (via campaign_reps)
...
Role enforcement is applied at the route level. A Talent hitting a brand endpoint returns `403 Forbidden`.
...
POST   /auth/parent-verify/:token      Parent clicks consent link — activates talent account
...
- If `date_of_birth` indicates age 16+: set `account_status = 'active'` immediately for talents
...
```
GET    /talents/me                        Own full profile
PUT    /talents/me                        Update profile fields
GET    /talents/me/profile-preview        Exactly what a recruiter/brand sees
GET    /talents/campaigns/available       Open campaigns matching talent's categories
GET    /talents/campaigns/active          Campaigns talent has accepted
GET    /talents/campaigns/history         Completed campaigns
GET    /talents/earnings                  Earnings breakdown: pending / confirmed / paid
```
...
**Parent approval gate:** if the Talent has a `parent_records` row with `campaign_approval_required = TRUE`, `/accept` returns `403 "awaiting parent approval"` (a distinct message, not a generic 403) until the linked parent calls `POST /parent/campaigns/:campaign_id/approve` (see [Section 9A](#9a-parent-portal)). `GET /talents/campaigns/available` also excludes any campaign whose category is in that parent's `values_filters` — the Talent never sees it as an option in the first place, so there's nothing to decline on that basis.
...
**Talent management within campaigns:**

```
GET    /brands/campaigns/:id/talents           List all talents on campaign with status
GET    /brands/campaigns/:id/talents/browse    Browse matched talent profiles
POST   /brands/campaigns/:id/talents/invite    Invite specific talent(s)
GET    /brands/campaigns/:id/talents/:talent_id/submission    View talent's submission
POST   /brands/campaigns/:id/talents/:talent_id/confirm       Confirm + trigger payout
POST   /brands/campaigns/:id/talents/:talent_id/revision      Request revision with note
POST   /brands/campaigns/:id/talents/:talent_id/rate          Submit rating
```
...
GET    /recruiters/talents/search         Search opted-in talent profiles (costs no credit)
GET    /recruiters/talents/:id            View full talent profile (costs 1 credit)
POST   /recruiters/talents/:id/contact    Send message to talent (costs 1 credit)
POST   /recruiters/talents/:id/save       Save talent to list
DELETE /recruiters/talents/:id/save       Remove from saved list
GET    /recruiters/saved               All saved talent profiles
```

**GET /recruiters/talents/search — Query params:**
...
```
GET    /admin/queue/talents               Pending talent approvals
GET    /admin/queue/brands             Pending brand approvals
GET    /admin/queue/recruiters         Pending recruiter approvals
POST   /admin/approve/:type/:id        Approve account (type: talent|brand|recruiter)
...
GET    /admin/analytics/talents           talent counts by city and category
...
The intelligence layer must operate on **anonymized, aggregated** data only. No individual Talent profile data — even internal data — may be used to generate intelligence reports. The pipeline is:

```
Campaign completion event
        ↓
Strip all PII (talent ID, name, school name)
        ↓
Aggregate to category + city + time period level
        ↓
Intelligence layer (minimum group size: 10 talents — never report on groups smaller than 10)
        ↓
Trend report output
```
...
A separate, deliberately narrow authenticated surface for parents/guardians of minor talents (see [Section 2.4](#24-the-parent-guardian-of-a-minor-talent--oversight-role-not-a-platform-side) for the role's scope and motivations, and [Section 7](#7-database-schema) for the `parent_records` / `parent_auth_tokens` / `campaign_reps.parent_approval_status` schema). Parents do not have `auth.users` accounts and are not a fourth platform side — this section exists because Teenure handles minors' data and income, not because it adds a new revenue stream.
...
- `request-link` looks up `parent_records` by `parent_email`. **Always returns the same response regardless of whether the email matches a record** — do not reveal whether a given email is linked to a minor talent's account (enumeration risk). Rate-limited per email, same pattern as `/auth/resend-consent`.
- If the email matches, a single-use token is created in `parent_auth_tokens` (15-minute expiry) and emailed via Resend.
- `verify/:token` validates the token (invalid/expired/already-used get distinct response s, same pattern as `/auth/parent-verify/:token`), marks it used, and issues a signed parent session token (24-hour expiry; see Section 7's schema note on `PARENT_SESSION_SECRET`).
...
```
GET    /parent/dashboard                        Linked talent's profile summary + earnings
...
POST   /parent/account/suspend                     Immediately suspend the talent account
...
- `campaign_approval_required` is **always `TRUE` and not parent-editable while the Talent is under 16** — `PUT /parent/settings/approval-required` returns `403` with an explanation if called for an out-of-range Talent age (under 16, or 18+ where the portal itself has already expired). It's only actually togglable for a 16-17-year-old talent.
- `POST .../block` never exposes the parent's reason to the brand — the campaign auto-declines with a neutral message ( "Talent is unavailable"), matching the existing decline flow's shape from the brand's point of view.
...
- `POST /parent/account/suspend` sets the talent's `account_status` to `suspended` immediately, notifies the talent, and alerts admin. `unsuspend` only works if the original suspension was parent-initiated — an admin-initiated suspension can only be reversed by admin.

### Compliance notes (extends Section 9's Non-Negotiable Requirements)

| Requirement                         | Implementation                                                                                                                                                                                                                                                                                                                      |
| ----------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **No enumeration via parent login** | `/parent/auth/request-link` gives an identical response whether or not the email is linked to a Talent — same principle as not confirming account existence elsewhere in the auth flow.                                                                                                                                             |
| **Parent portal data minimization** | The parent dashboard and digest are both allow-listed to specific fields (see above) — never a raw dump of the talent's activity. Recruiter message content and submission files are explicitly excluded from every parent-facing surface.                                                                                             |
| **Values-filter enforcement point** | Enforced server-side in `GET /talents/campaigns/available` (Section 8) — the Talent never sees a blocked-category campaign as an option. The parent portal's filter _configuration_ screen and the _enforcement_ point are different layers; configuring a filter with no enforcement would be a compliance gap, not just a UX one. |
| **Portal expiry at 18**             | Checked at every parent login/session-verify, not only at `parent_records` row creation, so a stale valid-looking session token can't outlive the talent's 18th birthday.                                                                                                                                                              |
...
- Payments: Stripe for brand billing, Stripe Connect for Talent payouts
...
3. Phase 1: Talent Portal (Section 5, Phase 1) — fully functional before Phase 2
...
- Parental consent email flow for under-16 talents (double opt-in, token-based, 72-hour expiry)
- FTC disclosure checkbox required before any Talent accepts any campaign
...
### Week 1–2: Build the Talent Side Manually

The cold start problem kills every platform. Solve the Talent side manually before the brand side exists.
...
- These are founding talents, not users. Treat them as co-founders of the network.
- Get commitment from each: two campaigns completed in first 60 days.
- Target: 10 talents in one city before approaching any brand.
...
- Pitch is one sentence: _"I have 10 verified student brand ambassadors at [school] ready to promote you this month for $300."_
- Three yeses from local brands funds first Talent payouts and proves the model.
...
### Month 2–3: Expand the Talent Network

- Each founding Talent recruits 3 friends. 10 becomes 30–40.
...
- Talent portal launches internally.

### Month 3–6: First College Conversation

- Target admissions officers at local colleges, not national universities.
- Pitch: _"We have 40 verified student profiles with documented real-world performance in [city]. Would your admissions team want early access?"_
...
| **Talent churn at graduation**        | Graduation is also your distribution channel. Every Talent who goes to college is a Teenure advocate on campus. Build an alumni Talent tier for 18–22 year olds explicitly.      |
...
| **School access restrictions**     | Never recruit on school property without permission. Work through students, not administration. The Talent recruits peers — you never go near the school directly.               |
...
| --- | ----------------------------------------------------------------------------- | --------------------------------------- |
| 1   | 10 talents with complete profiles in one city                                 | The network has a nucleus               |
...
| 6   | A Talent who joined at 14 uses their Teenure profile in a college application | The switching cost is real              |

---

> _The first time a college admissions officer asks for a Teenure profile is when you have a business. Everything before that is infrastructure._
