...
A three-sided verified-achievement platform for high school students, with three user types: Talent (teen), Brand, and Recruiter (college/employer). The one-sentence product rule from the spec: "Teenure is a verified professional achievement record for teenagers. Every feature either adds to that record or it does not belong on the platform." Apply this rule when evaluating any proposed feature — see Section 1 and Section 1A ("Content Policy") for the enforced exclusions (no talent-to-talent messaging, no public feed, no profile photos, no dating/discovery mechanics).
...
- Payments: Stripe (campaign billing) + Stripe Connect (talent payouts)
- Email: Resend
- Hosting: Vercel (frontend) + Railway (FastAPI)
- Analytics: PostHog

Planned monorepo layout is `apps/web` (Next.js, route groups per portal: `(talent)`, `(brand)`, `(recruiter)`, `(admin)`) and `apps/api` (FastAPI, routers per domain: auth/talents/brands/campaigns/recruiters/payments/admin), with `packages/shared-types` for cross-boundary TypeScript types. None of this exists on disk yet — treat Section 6 of the spec as the layout to create, not to discover.
...
- FTC sponsorship disclosure checkbox required before a Talent can accept any campaign
...
- Intelligence-layer data must be anonymized and aggregated (minimum group size of 10) before any trend report — never derived from individual Talent records
